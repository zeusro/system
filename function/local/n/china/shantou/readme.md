...
**Formula 1 — Average student score:**

$$
\bar{s} = \frac{\sum_i s_i}{N}, \quad N = \text{in-school student count}
$$

**Formula 2 — Undergraduate enrollment rate:**

$$
r = \frac{\text{enrolled}}{\text{exam-takers}} \times 100\%
$$
...
flowchart LR
    ...
    subgraph formulas["Formulas"]
        avg["avg score s̄(t) = S/N"]
        rate["enroll rate r(t) = E/X"]
    end
...
| School leader | $I(t)$ | Allocates resources (e.g. who gets bonus, who drops out; arranges for psychologist to provide targeted counseling); low $I(t)$ → pressure down; high → design incentives |
...
### 3.3 Multi-Agent Game Structure (GFM Diagram)
...
flowchart TB
    ...
    subgraph strategies["Strategy Space"]
        T --> T1["PUA / reduce exam / lie-and-evade / normal"]
        St --> S1["Dropout / cyberbully / athlete bonus / study hard / avoid"]
        Ps --> P1["Decompress / no action"]
        L --> L1["Pressure down / design incentive"]
    end
    subgraph consequences["Consequences ω"]
...
- **Leader:** Allocates resources (e.g. approves athlete bonus when choosing "design incentive"; arranges for psychologist to provide targeted counseling). If $I(t) < 0.5$ → pressure down; else design incentive. Student Y's athlete bonus takes effect only when leader chooses design incentive in that step.
...
#### Strategy Decision Flow (Teacher Y)

```mermaid
flowchart TD
    A[Teacher Y] --> B{LegalMoralRisk > 0.6?}
    B -->|Yes| C[lie-and-evade]
    B -->|No| D{Last-round defection & steps > 1?}
    D -->|Yes| E[normal teach]
    D -->|No| F{s̄ < 0.5 and N > 5?}
    F -->|Yes| G[PUA]
    F -->|No| E
```
...
### 5.2 Simulation Step Sequence (GFM Diagram)
...
sequenceDiagram
    ...
    Sim->>Context: Update SimContext (N, exam count, s̄, r, avg stress)
    loop For each active agent a
        Sim->>Agents: ChooseStrategy(t, a, ctx)
        Agents-->>Sim: strategy s
        Sim->>Agents: ApplyStrategy(t, s, a, ctx, rng)
        Agents->>Context: Apply consequences (ΔStress, Dropout, …)
    end
    Sim->>Log: Append LogTS (time + content)
    Sim->>Sim: Sample I(t) → (t, I(t))
```
