...
const (
	...
	RoleTeacherD                      // 教师D：公式1+2为依据，平均成绩最大化，但不采取剔除学生策略（y.md）
...
// NewAgent 构造社会成员 Agent，时间 t 为入校/入职时刻（时间第一成员）。
// 黑曼巴角色默认不在高考参考池，其余学生默认在参考池；StrategyCount 初始化为
// 长度为 NumStrategies 的切片；LegalRisk 初始为因子的 LegalMoralRisk。
