...
- **公式 1**：学生平均成绩 \(\bar{s} = \frac{\sum_i s_i}{N}\)（\(N\) 为在校学生人数）。
- **公式 2**：本科升学率 \(r = \frac{\text{本科录取人数}}{\text{参加高考人数}} \times 100\%\)。
...
其中 \(\bar{s}(t)\)、\(r(t)\) 由 \(t\) 时刻在校学生、参考人数与达线人数计算得到。
...
| 心理老师 | 系统平衡 | 减压安抚 |
| 学校领导 | \(I(t)\) | 负责分配学校资源（如谁加分、谁休学退学）；政绩低→向下施压；政绩高→设计激励 |
...
- **心理老师**：若平均压力 \(>0.4\) → 减压安抚。
- **领导**：负责分配资源（如本期选择「设计激励」即批准学生 Y 的运动员加分）。若 \(I(t) < 0.5\) → 向下施压；否则设计激励函数。学生 Y 的运动员加分仅在本步领导选择设计激励时生效。
